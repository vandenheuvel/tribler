...
class MultiChainDB(Database):
    ...
    def get_graph_edges(self, public_key, neighbor_level=1):
        ...
        values might not be the most recent one, as it is taken from a block which might not be the last one. To get the
        maximum total_up and total_down, make sure to take the maximum for a given node over different rows.

        :param public_key: the public key which has to be focused
        ...
        # Recursively query the higher order neighbors.
        for levelIndex in range(1, neighbor_level):
            query = u"""SELECT DISTINCT mc.public_key, mc.link_public_key FROM multi_chain mc,
                    (%(lastlevel)s) level%(index)d
                    WHERE level%(index)d.public_key = mc.public_key
                    OR level%(index)d.public_key = mc.link_public_key
                    OR level%(index)d.link_public_key = mc.public_key
                    OR level%(index)d.link_public_key = mc.link_public_key
                    """ % {"index": levelIndex, "lastlevel": query}
